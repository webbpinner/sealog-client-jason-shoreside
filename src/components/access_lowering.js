...
class AccessLowering extends Component {
  ...
  render() {
    ...
    const accessLoweringFormHeader = (<div>Lowering Access  - {this.props.lowering.lowering_id}</div>);
    ...
    if (this.props.roles && (this.props.roles.includes("admin") || this.props.roles.includes('lowering_manager'))) {

      return (
        <Panel className="form-standard">
          <Panel.Heading>{accessLoweringFormHeader}</Panel.Heading>
          <Panel.Body>
            <form onSubmit={ handleSubmit(this.handleFormSubmit.bind(this)) }>  
              <Field
                name="lowering_access_list"
...
AccessLowering = reduxForm({
  form: 'editLoweringAccess',
  enableReinitialize: true,
  validate: validate
})(AccessLowering);

export default connect(mapStateToProps, actions)(AccessLowering);